export class Dijkstra {
    ...
    findPath(grid, start, end) {
        ...
        // Dijkstra's algorithm doesn't use heuristic, so only initialize g for the start cell
        start.g = 0;

        while (this.openSet.length > 0) {
            // Find the cell in the open set with the lowest g value
            let lowestIndex = 0;
            for (let i = 1; i < this.openSet.length; i++) {
                if (this.openSet[i].g < this.openSet[lowestIndex].g) {
                    lowestIndex = i;
                }
            }
            ...
            for (let neighbour of neighbours) {
                if (this.closedSet.includes(neighbour) || neighbour.isBlocked) {
                    continue;
                }

                // Tentative g score (distance from start to neighbour)
    ...
    drawCell([cell, color, alternativeBorderColor, cellBorderColor, pathColor]) {
        ...
        if (this.closedSet.includes(cell)) {
            color = "rgba(0, " + 10 * cell.g + ", " + 10 * cell.g + ", 1)"; // red shade based on g value
